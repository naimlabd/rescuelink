...
import { AlertCircle, Clock, FileText, RefreshCw, Send, Flame, User, Volume2, VolumeX } from 'lucide-react';
...
const FireDashboard = ({ session, onSignOut }) => {
  ...
  const [responseForm, setResponseForm] = useState({
    ...
    status: 'acknowledged',
    eta_minutes: '6',
    worker_id: 'FIR-WORKER001',
    worker_name: session?.displayName || 'Fire Chief',
  });
  ...
  const fetchActiveIncidents = useCallback(async (isSocketEvent = false) => {
    try {
      ...
      const fireIncidents = allIncidents.filter(i => i.fire_required);
      
      if (isSocketEvent && alertsEnabled && fireIncidents.length > prevIncidentCount.current) {
        playAlert();
      }
      prevIncidentCount.current = fireIncidents.length;
      
      setIncidents(fireIncidents);
      setSelectedIncident((current) => {
        if (!current?.incident_id) return current;
        return fireIncidents.find((incident) => incident.incident_id === current.incident_id) || current;
      });
    } catch (error) {
  ...
  const counts = useMemo(() => ({
    ...
    pending: incidents.filter((incident) => incident.fire_status === 'pending').length,
    responding: incidents.filter((incident) => incident.fire_status === 'responding').length,
  }), [incidents]);
  ...
  const submitResponse = async () => {
    ...
    try {
      ...
      setActionNotice('Fire brigade status update sent.');
      setResponseForm((current) => ({ ...current, response_message: '' }));
      await fetchActiveIncidents();
    } catch (error) {
      console.error('Error submitting response:', error);
      window.alert('Unable to send the fire department update right now.');
    }
  };
  ...
  return (
    <PortalShell
      role="fire"
      eyebrow="Multan Fire Brigade"
      title="Fire & Disaster Control"
      subtitle="Fire and massive disaster incidents requiring brigade response appear here."
      session={session}
      onSignOut={onSignOut}
      actions={
        <>
          <button
            ...
            className={`inline-flex items-center rounded-xl border px-4 py-2 text-sm font-medium backdrop-blur transition ${
              alertsEnabled ? 'border-orange-500/50 bg-orange-500/20 text-orange-300' : 'border-[var(--glass-border)] bg-white/10 text-white hover:bg-white/20'
            }`}
          >
        ...
        <div className="space-y-6">
          ...
          <div className="glass-panel p-5">
            <div className="mb-4">
              ...
              <h2 className="mt-1 text-2xl font-bold text-white">Active fire cases</h2>
            </div>
            {loading ? (
                  ...
                  >
                    <div className="flex items-start justify-between gap-3">
                      <div className="flex items-center gap-2">
                        ...
                        <span className={`rounded-full px-3 py-1 text-xs font-semibold ${statusStyles[incident.fire_status] || statusStyles.pending}`}>
                    ...
                    <div className="mt-3 flex flex-wrap gap-4 text-xs text-slate-400">
                      <span className="inline-flex items-center gap-1"><User className="h-3.5 w-3.5 text-orange-400" />{incident.citizen_name || 'Anonymous'}</span>
                      <span className="inline-flex items-center gap-1"><Clock className="h-3.5 w-3.5 text-orange-400" />ETA {incident.eta_minutes || 0} min</span>
                    </div>
                  </button>
                ))}
              </div>
              ...
              <div className="rounded-2xl bg-white/5 p-8 text-center">
                ...
                <div className="text-sm text-slate-400">No active fire cases at the moment.</div>
              </div>
            )}
          </div>
        </div>
              ...
              <div className="space-y-5 animate-fade-in">
                <div className="grid gap-4 sm:grid-cols-2">
                  {[
                    ...
                    { title: 'Assigned Unit', val1: selectedIncident.assigned_fire_unit, val2: 'Fire Command' },
                ...
                <MapPreview lat={selectedIncident.lat} lon={selectedIncident.lon} secondaryLat={selectedIncident.fire_location?.lat} secondaryLon={selectedIncident.fire_location?.lon} title="Incident Location" detail={selectedIncident.fire_location ? `Unit ${selectedIncident.fire_location.label}` : ''} />
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                  <label className="block">
                    <span className="mb-2 block text-sm font-medium text-slate-300">Fire status</span>
                    <select
                      ...
                      className="w-full rounded-2xl border border-[var(--glass-border)] bg-slate-900/50 px-4 py-3 text-white outline-none focus:border-orange-500 focus:ring-1 focus:ring-orange-500"
                    >
                      <option value="acknowledge">Unit assigned</option>
                      <option value="respond">En route</option>
                      <option value="update">Fighting fire / Active</option>
                ...
                <label className="block">
                  ...
                  <textarea
                    ...
                    placeholder="Example: Fire engine is on the way."
                    rows={3}
                    className="w-full rounded-2xl border border-[var(--glass-border)] bg-slate-900/50 px-4 py-3 text-white outline-none focus:border-orange-500 focus:ring-1 focus:ring-orange-500"
                  />
                </label>
                ...
                <button
                  ...
                  className="inline-flex w-full items-center justify-center rounded-2xl bg-orange-600 px-5 py-4 text-sm font-semibold text-white transition hover:bg-orange-500 hover:shadow-[0_0_20px_rgba(234,88,12,0.4)]"
                >
                  <Send className="mr-2 h-4 w-4" />
                  Send fire brigade update
                </button>
                {actionNotice && (
